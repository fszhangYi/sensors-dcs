    ...
    (release / "README.txt").write_text(
        "\n".join(
            [
                ...
                "",
                "1. Unzip this folder",
                "2. Double-click sensors-dcs.exe",
                "3. No need to install Python / Node",
                "4. User data: %APPDATA%\\sensors-dcs\\",
                "5. Default config: %APPDATA%\\sensors-dcs\\configs\\gello_only.yaml",
                "6. dry_run=true by default; set SENSORS_DCS_DRY_RUN=0 for hardware",
                "7. Force browser UI: SENSORS_DCS_BROWSER=1",
                "8. Desktop window needs Microsoft Edge WebView2 Runtime",
                "9. Hardware: dynamixel-sdk + pyserial bundled (Gello)",
                "   (USB serial driver still required on the Windows host)",
                "",
            ]
        ),
        encoding="utf-8",
